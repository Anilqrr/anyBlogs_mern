...
export default function Search() {
  ...
  const { blog, searchdata, pending } = useSelector(state => state.blog)
  ...
  return (
    <>
      <div className="search">
        <form method='post'>
          <input type="text" name="search" value={search || ""} onChange={handleCahnge} id="search" placeholder='Search anyBlogs....' />
          <button type='submit' onClick={(e) => {
            e.preventDefault()
            search !== "" && dispatch(SearchBlog(search))
            search !== "" && navigate('/blog/search')
            setSearch("")
